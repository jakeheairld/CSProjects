...
def main(mesh_path, output_dir):
    ...
    # Read mesh
    try:
        domain, cell_markers, facet_markers = gmshio.read_from_msh(mesh_path, MPI.COMM_WORLD, gdim=gdim)
    ...
    np.savetxt(os.path.join(output_dir, "C_hom.txt"), C_hom)
    print(f"Results saved to {output_dir}")

if __name__ == "__main__":
    mesh_path = sys.argv[1] if len(sys.argv) > 1 else "meshes/xdmf_meshes/mesh1.msh"
    output_dir = sys.argv[2] if len(sys.argv) > 2 else "results/default"
