...
public class GrayscaleImage {
    ...
    public double getPixel(int x, int y){
    	if((y > imageData.length - 1 || x > imageData[0].length - 1) || (y < 0 || x < 0)) {
    		throw new IllegalArgumentException("Entered x and/or y value(s) was not within image width/height.");
    	}
    	double pixelData = imageData[y][x];
        return pixelData;
    }
    ...
    public boolean equals(Object other){
        ...
        //Checks if both images have the same amount of rows
        if(this.imageData.length != otherImage.imageData.length) {
        	return false;
        } 
        //Checks if the length of all rows is the same between both images
        if(this.imageData[0].length != otherImage.imageData[0].length) {
        	return false;
        }
        //Checks to ensure all pixels are exactly equal
    ...
    public double averageBrightness(){
    	int numPixels = 0;
    	double total = 0;
    	double averagePixelData;
    	for(int row = 0; row < this.imageData.length; row++) {
        	for(int col = 0; col < this.imageData[row].length; col++) {
        		total += this.getPixel(col, row);
        		numPixels++;
        	}
    	}
    	averagePixelData = total/numPixels;
    ...
    public GrayscaleImage normalized(){
    	double ratio = 127.0/this.averageBrightness();
    	double[][] normalizedImageData = new double[this.imageData.length][this.imageData[0].length];
    	for(int row = 0; row < normalizedImageData.length; row++) {
    		for(int col = 0; col < normalizedImageData[0].length; col++) {
    			normalizedImageData[row][col] = this.getPixel(col, row) * ratio;
    		}
    	}
    	GrayscaleImage normalizedImage = new GrayscaleImage(normalizedImageData);
    ...
    public GrayscaleImage mirrored(){
    	double[][] mirroredImageData = new double[this.imageData.length][this.imageData[0].length];
    	for(int row = 0; row < mirroredImageData.length; row++) {
    		for(int col = 0; col < mirroredImageData[0].length; col++) {
    			mirroredImageData[row][col] = this.getPixel(mirroredImageData[0].length-1 - col, row);
    		}
    	}
    	GrayscaleImage mirroredImage = new GrayscaleImage(mirroredImageData);
    ...
    public GrayscaleImage cropped(int startRow, int startCol, int width, int height){
    	if(startRow < 0 || startCol < 0) {
    		throw new IllegalArgumentException("Starting row or starting column entered must be zero or greater.");
    	}
    	if((startRow + height > this.imageData.length) || (startCol + width > this.imageData[0].length)) {
    		throw new IllegalArgumentException("Width or length entered is outside of image size.");
    	}
    	double[][] croppedImageData = new double[height][width];
    	for(int row = startRow; row < startRow + height; row++) {
    		for(int col = startCol; col < startCol + width; col++) {
    			croppedImageData[row - startRow][col - startCol] = this.getPixel(col, row);
    		}
    	}
    	GrayscaleImage croppedImage = new GrayscaleImage(croppedImageData);
        return croppedImage;
    }
    ...
    public GrayscaleImage squarified(){
    	...
    	if(this.imageData.length > this.imageData[0].length) {
    		sideLength = this.imageData[0].length;
    	} else {
    		sideLength = this.imageData.length;
    	}
    	double[][] squarifiedImageData = new double[sideLength][sideLength];
    	for(int row = 0; row < sideLength; row++) {
    		for(int col = 0; col < sideLength; col++) {
    			squarifiedImageData[row][col] = this.getPixel(col, row);
    		}
    	}
    	GrayscaleImage squarifiedImage = new GrayscaleImage(squarifiedImageData);
